...
BOOL CPing::Ping3(LPCTSTR pszHostName, CPingReply& pr, UCHAR /*nTTL*/, DWORD dwTimeout, UCHAR nPacketSize, CString csSourceIP) const
{
  //Parameter validation
  if (nPacketSize > MAX_ICMP_PACKET_SIZE || nPacketSize < MIN_ICMP_PACKET_SIZE)
  {
  ...
  if (sockRaw == INVALID_SOCKET) 
    ...
    int set_on = 0;
    if (setsockopt( sockRaw, SOL_SOCKET, SO_DONTROUTE,(const char *)&set_on, sizeof(DWORD) ) < 0 )
        return FALSE;

	///////////////////////////////////////////////////////////////
	//Bind IP here 
	//Add by Fan Jen-Chieh 2006/9/5
	...
	if ( csSourceIP == "any" )
		...
		source_addr.sin_addr.s_addr = inet_addr(csSourceIP);
	source_addr.sin_port = htons(0); //¥ô·Nport

	iResult = bind( sockRaw,					//socket
					(LPSOCKADDR)&source_addr,	//Our address
	...
	iResult = gethostname( szServer, sizeof(szServer) );
